...
st.set_page_config(page_title="登录注册系统", page_icon="🚀" )
...
# 设置页面标题
st.title('A Random App with Wave Background')
# 显示一些文本
...
if radio_option == "新增账户":
    st.subheader("新增凭据 🗝️")
    st.markdown("####")
    app_name = st.text_input('应用 📱', '推特')
    user_name = st.text_input('用户名 👤', '大学生只会吃饭')
    pass_word = st.text_input('密码 🔑', '123456', type="password", )
    email = st.text_input('电子邮件 📧', 'tweety@xyz.com')
    url = st.text_input('网站 🔗', 'twitter.com')
    st.markdown("####")
